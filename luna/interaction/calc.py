...
class InteractionsManager:
    ...
    interactions : iterable of
        ...
        An initial sequence of
            :class:`~luna.interaction.type.InteractionType` objects.
    ...
    def interactions(self):
        ...
        Additional interactions should be added using the
        method :py:meth:`add_interactions`."""
    ...
    def count_interations(self, must_have_target=False):
        ...
        must_have_target : bool
                ...
                The default value is False, which implies all
    ...
    def filter_by_types(self, types):
        """Filter :class:`~luna.interaction.type.InteractionType`
    ...
    def filter_out_by_binding_mode(self, binding_modes_filter):
        ...
        **Note:** this method modifies ``interactions``.
        ...
        binding_modes_filter : \
            ...
            A :class:`~luna.interaction.filter.BindingModeFilter` object
            that defines binding mode conditions to decide which
            interactions are valid.
    ...
    def to_json(self, output_file=None, indent=None):
        ...
        indent : int or str, optional
            ...
            If a string is provided (e.g., '\\\\t'), it will be used to
            indent each level. The default value is None, which selects
    ...
    def save(self, output_file, compressed=True):
        """Write the pickled representation of the `InteractionsManager`
        object to the file ``output_file``.
        ...
        compressed : bool, optional
            If True (the default), compress the pickled representation
            as a gzip file (.gz).
    ...
    @staticmethod
    def load(input_file):
        """Load the pickled representation of an `InteractionsManager`
        object saved at the file ``input_file``.
...
class InteractionCalculator:
    ...
    .. note::
        In case you want to disable specific parameters (e.g., angles)
        used during the calculation of interactions, you do not need to
        define a custom function for it. You could just delete the parameter
        from the configuration and LUNA will automatically recognize that a
        given parameter is not necessary anymore.
    ...
    inter_filter : :class:`~luna.interaction.filter.InteractionFilter`, \
            optional
        An :class:`~luna.interaction.filter.InteractionFilter` object to
        filter out interactions on-the-fly. The default value is None,
        which implies no interaction will be filtered out.
    inter_funcs : dict of {tuple : iterable of callable}
        ...
        (e.g. ``("Hydrophobic", "Hydrophobic")``) and values are lists
        of references to custom functions (see Examples for more details).
    ...
    add_non_cov : bool
         ...
         interactions and want to make them controllable by this flag,
         make sure to verify the state of ``add_non_cov`` at the beginning
    ...
    add_atom_atom : bool
        ...
        beginning of the function and return an empty list in case
        it is False.

        .. note::
            In LUNA, we consider the following interactions as
            atom-atom: `Van der Waals`, `Van der Waals clash`, and
            `Atom overlap`. We opted to separate `Van der Waals` from other
            non-covalent interactions because LUNA may generate an unnecessary
            number of additional interactions that are usually already
            represented by other non-covalent interactions as weak hydrogen
            bonds, hydrophobic, or dipole-dipole interactions. Thus, to give
            users a fine-grain control over which interactions to calculate,
            we provided this additional flag to turn off the calculation of
            Van der Waals interactions.
    add_dependent_inter : bool
        If True, compute interactions that depend on other interactions.
        Currently, only water-bridged hydrogen bonds and salt bridges have
        a dependency on other interactions. The first, depends on two or
        more hydrogen bonds, while the second depends on an ionic and a
        hydrogen bond. The default value is False, which implies no
        dependent interaction will be computed.
    ...
    strict_donor_rules : bool
        If True (the default), hydrogen bonds will only be considered for
        donor atoms with explicit hydrogens bound to them. In that case,
        angles and distances will be evaluated. However, if the molecule
        containing the donor atom is in ``lazy_comps_list``, then angles
        and hydrogens will be ignored and LUNA will proceed with the
        determination of hydrogen bonds based only on donor-acceptor
        distances. Another exception occurs for solvent molecules in which
        the donor atom is only bound to hydrogens atoms (e.g., water, ammonia,
        and hydrogen sulfide). In that case, hydrogens can be positioned in
        many different ways by Open Babel, which may cause LUNA to detect
        different hydrogen bonds at each run. So, to circumvent this problem,
        by default, LUNA always ignores the explicit hydrogen position
        for donor atoms that only contain hydrogens bound to it.
    strict_weak_donor_rules : bool
        If True (the default), weak hydrogen bonds will only be considered
        for donor atoms with explicit hydrogens bound to them. In that case,
        angles and distances will be evaluated.
    ...
    lazy_comps_list : iterable
         ...
         The default list is ['HOH', 'DOD', 'WAT', 'H2O', 'OH2', 'NH3',
         'NH4'], which only contains water, ammonia, and ammonium ion,
    ...
    **Example 1) How to define custom interactions:**
    ...
    :class:`~luna.interaction.type.InteractionType` object is created with
    type `Hydrogen bond`.
    ...
    .. note::
        Observe that the function checks if ``add_non_cov`` has been
        turned off and if so returns an empty list. That's a recommended
        strategy because it allows one to turn off all non-covalent
        interactions with a single flag.

        Also, observe that `InteractionCalculator` always expects functions
        to return a list at the end. That means multiple interactions may
        be detected for a single pair of :class:`~luna.mol.groups.AtomGroup`
    ...
    **Example 2)** How to modify parameters to calculate interactions:
    ...
    :class:`~luna.interaction.config.DefaultInteractionConfig`, alter
    parameters, and pass it to `InteractionCalculator`.
    ...
    Finally, if you want to define a custom configuration that will be used
    in your custom functions, you first need to define the parameters as a
    dict and then initiate a new
    ...
    **Example 3)** How to disable specific parameters and how to enable
    ...
    exist in ``inter_config``. Thus, you can take advantage of this system
    when implementing custom functions so others will also have the
    possibility to turn off specific parameters without modifying the code
    directly. Let's see that in practice.
    ...
    Finally, we are ready to provide the function and custom
    parameters to `InteractionCalculator`.
    ...
    def calc_interactions(self, trgt_atm_grps, nb_atm_grps=None):
        """Calculate interactions established by atoms and atoms groups
        in ``trgt_atm_grps`` using methods available in ``funcs``.
        ...
        function defined for the tuple ``("Hydrophobic", "Hydrophobic")``
        in ``funcs``. Consider now a pair of
        :class:`~luna.mol.groups.AtomGroup` objects whose features are 'Donor'
        and 'Hydrophobic'. Once again, `calc_interactions` will evaluate if
        there is any function defined for the tuple
        ``("Donor", "Hydrophobic")`` (the order does not matter).
        ...
        # If nb_atm_grps was not informed, it uses the trgt_atm_grps
        # as the neighbors. In this case, the interactions will be
        # target x target.
        ...
        for trgt_atm_grp in trgt_atm_grps:
            for nb_atm_grp in ss.search(trgt_atm_grp.centroid, bsite_cutoff):
                ...
                # groups. Loops in the graph is not permitted and does
                # not make any sense.
                ...
                is_intramol_inter = self._is_intramol_inter(trgt_atm_grp,
                                                            nb_atm_grp)
                ...
                for pair in feat_pairs:
                    ...
                    # producing distances lower than their sum of
                    # Van der Waals radius. As a consequence the algorithm
                    # will find a lot of false interactions.
                    #
                    ...
                    if (pair[0].name != "Atom" and pair[1].name != "Atom"
                        ...
                        if shortest_path_length is None:
                            ...
                            # So, if two groups return INF, it means they
                            # are a valid combination as they match the
                            # minimum bond separation.
                            get_func = self.inter_config.get
                            cutoff = get_func("min_bond_separation", 0)
                            path_func = trgt_atm_grp.get_shortest_path_length
                            shortest_path_length = path_func(nb_atm_grp,
                                                             cutoff)
                        ...
                        # cutoff 'min_bond_separation'. Therefore, ignore them.
    ...
    def find_dependent_interactions(self, interactions):
        ...
        Currently, only water-bridged hydrogen bonds and salt bridges
        have a dependency on other interactions. The first, depends
        on two or more hydrogen bonds, while the second depends on an
        ionic and a hydrogen bond. The default value is False, which
        implies no dependent interaction will be computed.
        ...
        for hbond, ionic in product(hbond_set, ionic_set):
            ...
            # acceptor atom belongs to a negative and positive group at
            # the same time.
            if condA ^ condB:
                ...
                inter = InteractionType(ionic.src_grp, ionic.trgt_grp,
                                        "Salt bridge", params=params)
                dependent_interactions.add(inter)
    ...
    def remove_inconsistencies(self, interactions):
        ...
        .. note::
            ...
            Due to the strength of a hydrogen bond, atoms may approximate
            more to each other, which sometimes may cause unfavorable
            interactions involving dipoles to be detected. To avoid such
            conflicts, LUNA removes the unfavorable interactions.
    ...
    def calc_cation_pi(self, params):
        ...
        params : tuple of (:class:`~luna.mol.groups.AtomGroup`,\
                           ...
                           :class:`~luna.mol.features.ChemicalFeature`)
            The tuple follows the order
            (:math:`A`, :math:`B`, :math:`A_f`, :math:`B_f`), where
            :math:`A` and :math:`B` are two :class:`~luna.mol.groups.AtomGroup`
            objects, and :math:`A_f` and :math:`B_f` are their features
    ...
    def calc_hydrop(self, params):
        ...
        # Verify if the groups contain the required number of atoms
        # to form a valid surface.
        ...
        # Verify if the number of interacting atoms attends the required
        # number of interating atoms per surface.
        if (not self.is_within_boundary(len(interacting_atms_in_surf1),
                                        "min_inter_atom_in_surf", ge)
                or not self.is_within_boundary(len(interacting_atms_in_surf2),
                                               "min_inter_atom_in_surf", ge)):
            return []
        ...
        if (self.is_within_boundary(min_cc_dist, "bsite_cutoff", le)
            ...
            inter = InteractionType(group1, group2, "Hydrophobic",
                                    params=params)
            interactions.append(inter)
    ...
    def calc_ion_multipole(self, params):
        ...
        if (self.is_within_boundary(id_dist, "bsite_cutoff", le)
            ...
            if len(dipole_grp.atoms) == 2:
                ...
                # interest (the electrophile or nucleophile), and Y is its
                # counterpart.
                y_atm = (dipole_grp.atoms[1]
                         if dipole_grp.atoms[0] == part_charged_atm
                         else dipole_grp.atoms[0])
            ...
            prop_name = "min_idy_ang_ion_multipole_inter"
            if (len(dipole_grp.atoms) == 1
                    or self.is_within_boundary(idy_angle, prop_name, ge)):
    ...
    def calc_multipolar(self, params):
        """Default method to calculate favorable and unfavorable
        ...
        if (len(group1.atoms) != 1 and len(group1.atoms) != 2
            ...
            logger.warning("A dipole group should have 1 (for cases when the "
                           ...
                           % (group1, group2, len(group1.atoms),
                              len(group2.atoms)))
            return []
        ...
        # depend on the number of atoms in the dipoles. With dipoles
        # containing 1 atom, it takes a generous approach by ignoring angles
        # and accepting everything. With dipoles containing two atoms, it
        # requires that at least one of the angles fits the rules to be
        # accepted.
        elif (feat1.name == feat2.name
                and (feat1.name == "Nucleophile"
                     or feat1.name == "Electrophile")):
            # If only one group contains 1 atom, use it as the dipole 2
            # because it is used as the reference to calculate the NEY angle.
            # Since we take a generous approach, with one atom no angle will be
            # calculated and the interaction will be accepted.
            if len(group1.atoms) == 1 and len(group2.atoms) == 2:
                dipole_grp1, dipole_type1 = group2, feat2.name
                dipole_grp2, dipole_type2 = group1, feat1.name
            # All the other number combinations ([2,1], [1,1], [2, 2])
            # come here.
            else:
                dipole_grp1, dipole_type1 = group1, feat1.name
                dipole_grp2, dipole_type2 = group2, feat2.name
        ...
        # Atom 2 => Dipole 2
        #
        # An electrophile may have only 1 atom.
        # E.g.: NH4, although by default we consider it as an ion.
        ...
        # the method for them are equal to the favorable interaction. So, from
        # now on, we will deal with them as if it was the first model.
        #
        ...
        if (self.is_within_boundary(ne_dist, "bsite_cutoff", le)
            ...
            # No angle can be calculated if the electrophile (dipole 2)
            # has only one atom.
                ...
                for d1, d2 in combinations:
                    ...
                    if (self.is_within_boundary(ney_angle, prop_name1, ge)
                        ...
                        if self.is_within_boundary(disp_angle, prop_name, le):
                            ...
                            # able to calculate the angle between the vectors
                            # AN and EY. This angle is necessary to define the
    ...
    def calc_xbond_pi(self, params):
        """Default method to calculate halogen bonds between halogens
        and aromatic rings.
        ...
        params : tuple of (:class:`~luna.mol.groups.AtomGroup`,\
            ...
            The tuple follows the order (:math:`A`, :math:`B`, :math:`A_f`,
            :math:`B_f`), where :math:`A` and :math:`B` are two
            :class:`~luna.mol.groups.AtomGroup` objects, and
            :math:`A_f` and :math:`B_f` are their features
            (:class:`~luna.mol.features.ChemicalFeature` objects),
            respectively.
        ...
        if (self.is_within_boundary(xa_dist, "bsite_cutoff", le)
            ...
            if (self.is_within_boundary(disp_angle,
                ...
                # group. In such cases the halogen may also form more than one
                # halogen bond.
    ...
    def calc_xbond(self, params):
        ...
        if len(group1.atoms) != 1 or len(group2.atoms) != 1:
            logger.warning("One or more invalid atom groups were informed: %s "
                           "and %s. In halogen bonds, halogen donor and "
                           "acceptor groups should always contain only one "
                           "atom." % (group1, group2))
            return []
    ...
    def calc_chalc_bond(self, params):
        ...
        if (self.is_within_boundary(ya_dist, "bsite_cutoff", le)
            ...
            n_coords = [nbi.coord for nbi in acceptor_atm.neighbors_info
                        if nbi.atomic_num != 1]
            ...
            for r_atm in r_atms:
                ...
                if (self.is_within_boundary(rya_angle,
                    ...
                    # that only hydrogens may be bound to it. Then, we do not
                    # calculate the angles because hydrogens are too dynamic,
                    # i.e., the acceptor could be ionized or not at a specific
                    # moment in time and its hydrogens may be positioned in
                    # different ways.
    ...
    def calc_chalc_bond_pi(self, params):
        """Default method to calculate chalcogen bonds between chalcogens
        and aromatic rings.
    ...
    def calc_hbond(self, params):
        ...
        if len(group1.atoms) != 1 or len(group2.atoms) != 1:
            logger.warning("One or more invalid atom groups were informed: %s "
                           "and %s. In hydrogen bonds, donor and acceptor "
        ...
        if (self.is_within_boundary(da_dist, "bsite_cutoff", le)
            ...
            hydrog_coords = [nbi.coord for nbi in donor_atm.neighbors_info
                             if nbi.atomic_num == 1]
            ...
            # Firstly, it checks if it is not necessary to apply a strict hbond
            # rule, i.e., hydrogens must exist and all geometrical criteria
            # should be evaluated. Then it checks if no hydrogen is bonded to
            # the donor, or if the donor has hydrogens and only hydrogens as
            # neighbours (water, solvents, ammonia, SH2). In the latter case,
            # the hydrogens can be positioned in many different ways, and each
            # run of a tool like OpenBabel would vary the hydrogen bond list
            # when one applies this algorithm.
            #
            ...
            # the list contains only Water molecules.
            if ((self.strict_donor_rules is False
                    and (len(hydrog_coords) == 0
                         or len(hydrog_coords)
                         == len(donor_atm.neighbors_info)))
                    or (donor_atm.parent.resname in self.lazy_comps_list)):

                # When the position of the hydrogen cannot be defined, it
                # assumes the hydrogen to be located 1 A away from the donor in
                # a line formed by the donor and the acceptor.
                ha_dist = da_dist - 1
                if self.is_within_boundary(ha_dist, "max_ha_dist_hb_inter",
                    ...
                    if len(r_coords) == 0:
                        ...
                        prop_name = "min_dar_ang_hb_inter"
                ...
                for h_coord in hydrog_coords:
                    ...
                    if (self.is_within_boundary(ha_dist,
                        ...
                        if len(r_coords) == 0:
                            ...
                            # satisfy the criteria. OBS: it may happen that
                            # each one of the angles would belong to a
                            # different R atom.
    ...
    def calc_weak_hbond(self, params):
        ...
        if len(group1.atoms) != 1 or len(group2.atoms) != 1:
            logger.warning("One or more invalid atom groups were informed: %s "
                           "and %s. In weak hydrogen bonds, weak donor and "
                           "(weak) acceptor groups should always contain only "
                           "one atom." % (group1, group2))
            return []

        if ((feat1.name == "Acceptor"
             or feat1.name == "WeakAcceptor")
                and feat2.name == "WeakDonor"):

            donor_grp = group2
            acceptor_grp = group1

        elif (feat1.name == "WeakDonor"
                and (feat2.name == "Acceptor"
        ...
        if (self.is_within_boundary(da_dist, "bsite_cutoff", le)
            ...
            # By default, the list contains only water, ammonia,
            # and ammonium ion.
            if ((self.strict_weak_donor_rules is False
                ...
                # When the position of the hydrogen cannot be defined, it
                # assumes the hydrogen to be located 1 A away from the donor
    ...
    def calc_hbond_pi(self, params):
        ...
        if (feat1.name == "Aromatic"
                and (feat2.name == "Donor"
                     or feat2.name == "WeakDonor")):
            ring_grp = group1
            donor_grp = group2

        elif (feat2.name == "Aromatic"
                and (feat1.name == "Donor"
                     or feat1.name == "WeakDonor")):
            ring_grp = group2
            donor_grp = group1
        ...
        if (self.is_within_boundary(da_dist, "bsite_cutoff", le)
            ...
            # the donor, or if the donor has hydrogens and only hydrogens as
            # neighbours (water, solvents, ammonia, SH2). In the latter case,
            # the hydrogens can be positioned in many different set of ways,
            # and each run of a tool like OpenBabel would vary the hydrogen
            # bond list when one applies this algorithm.
            #
            # If the user has also defined a list of lazy compounds, we can
            # skip the application of strict rules on them as well.
            ...
            if ((self.strict_weak_donor_rules is False
                ...
                for h_coord in hydrog_coords:
                    ...
                    if (self.is_within_boundary(ha_dist, prop_name1, le)
                        ...
                        # Interaction model: D-H ---- A, in which A is the
                        # ring center. Calculate the displacement angle formed
    ...
    def is_within_boundary(self, value, key, func):
        ...
        .. note::
            It will always return True if the parameter does not exist
            in ``inter_config``.
    ...
    def set_functions_to_pair(self, pair, funcs):
        """Set functions to calculate interaction for the given pair
        of features.
