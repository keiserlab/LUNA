...
class ShellManager:
    ...
    levels : dict of {int: list of `Shell`}
        Register shells by level, where keys are levels and values are lists
        of `Shell` objects.
    ...
    def __init__(self, num_levels, radius_step, fp_length, ifp_type,
                 shells=None, verbose=False):
    ...
    def add_shell(self, shell):
        ...
        # Any new shell without interactions from level 1 onwards will
        # be automatically considered invalid.
    ...
    def get_valid_shells(self):
        ...
        substructural information. That means this shell is not unique and
        does not contributes to any new information.
    ...
    def get_shell_by_center_and_level(self, center, level,
        ...
        (:class:`~luna.mol.groups.AtomGroup` object) at level
        (iteration) ``level``.
        ...
        unique_shells : bool
            If True, return the `Shell` object if it is unique and
            None otherwise. The default value is False.
    ...
    def to_fingerprint(self, fold_to_length=None, count_fp=False,
                       unique_shells=False):
        ...
        if count_fp:
            fp = CountFingerprint(indices, fp_length=self.fp_length,
                                  props=props)
        else:
    ...
    def trace_back_feature(self, feature_id, ifp, unique_shells=False):
        """Trace a feature from a fingerprint back to the shells that
        originated that feature.

        .. note::
            Due to fingerprint folding, multiple substructures may end up
            encoded in the same bit, the so-called collision problem. So, if
            the provided feature contains collisions, shells representing
            different substructures may be returned by
        ...
        Now, we can trace features back to original identifiers and
        investigate its substructural information.
...
<ExtendedAtom: 3QQK/0/A/GLN/85/OE1>]>, interactions=0>])]
        ...
        for ori_feature in ifp.unfolding_map[feature_id]:
            yield (ori_feature,
                   self.get_shells_by_identifier(ori_feature,
                                                 unique_shells))
...
class Shell:
    ...
    """
    A container to store substructural information, which is the base for LUNA
    fingerprints.
    ...
    inter_tuples : iterable of \
        ...
        .. note::
            ...
            `ShellGenerator` sorts atom groups and considers only the first
            tuple that appears, which guarantees that only one of the possible
            tuples is added to avoid information duplication.
    ...
    valid : bool
        If the shell is valid or not. By default, all shells are
        considered valid.
    ...
    def __init__(self, central_atm_grp, level, radius, neighborhood=None,
                 inter_tuples=None, diff_comp_classes=True, dtype=np.int64,
                 seed=0, manager=None, valid=True, feature_mapper=None):
        ...
        # Encode the interactions. It represents the substructures comprised
        # by this shell.
    ...
    def identifier(self):
        ...
        hashing its encoded data with a hash function.
        By default, LUNA uses MurmurHash3 as the hash function."""
    ...
    def previous_shell(self):
        """`Shell`, read-only: The previous shell, i.e., a shell centered on
        the same central :class:`~luna.mol.groups.AtomGroup` object from a
        previous level. For example, if this shell is in level 5, return a
        shell from level 4 having the same center."""
    ...
    def is_similar(self, shell):
        ...
        # The shells' identifiers will be equal when the shells encode the
        # same information and were constructed in the same level.
        ...
        # If both shells have interactions, check if the interactions
        # are equal.
        ...
        if self.level == 0:
            ...
            for (inter, nb_atm_grp) in self._inter_tuples:
                if isinstance(nb_atm_grp, PseudoAtomGroup):
                    ...
                    # a specific interaction don't generate shells. Only their
                    # parents do, i.e., the whole group generates shells.
                    # So, use it instead.
                    nb_atm_grp = nb_atm_grp.parent_grp

                prev_nb_shell = self._manager.get_previous_shell(nb_atm_grp,
                                                                 self.level)
            ...
            # Sort the tuples to avoid dependence on the order in which
            # tuples are added.
    ...
    def _initial_shell_data(self):
        ...
        # FIFP uses pharmacophore properties for atoms and atoms' group.
            ...
            if len(self.central_atm_grp.atoms) == 1:
                ...
                # decide to remove the If, then the code for capturing all
                # atom derived features will be already working.
                for atm in self.central_atm_grp.atoms:
        ...
        # belonging to Residues, Nucleotides, Ligands, or Waters are treated
        # as being different even when the group would be considered the same.
    ...
    def _encode_interactions(self):
        ...
        for inter in self.interactions:
            ...
            init_trgt_shell = self._manager.get_previous_shell(inter.trgt_grp,
                                                               1)
    ...
    def __repr__(self):
        return ("<Shell: level=%d, radius=%f, center=%s, interactions=%d>"
                % (self.level, self.radius, self.central_atm_grp,
...
class ShellGenerator:
    ...
    increased by 3 and substructural information will be encoded following
    EIFP definition.
...
873  882
    ...
    def __init__(self, num_levels, radius_step, fp_length=DEFAULT_FP_LENGTH,
                 ifp_type=IFPType.EIFP, diff_comp_classes=True,
                 dtype=np.int64, seed=0, bucket_size=10):
    ...
    def create_shells(self, atm_grps_mngr):
        ...
        atm_grps_mngr : :class:`~luna.mol.groups.AtomGroupsManager`
            Container of :class:`~luna.mol.groups.AtomGroup` objects and
            their interactions.
        ...
        sm = ShellManager(self.num_levels, self.radius_step,
                          self.fp_length, self.ifp_type)
        ...
        for level in range(self.num_levels):
            ...
            for atm_grp in sorted_neighborhood:
                ...
                # Ignore centroids that already reached the limit of
                ...
                if radius > 0:
                    ...
                    if not prev_shell:
                        logger.exception("No previous shell centered in %s "
                                         "was found. " % atm_grp)
                    ...
                    # Get valid derived groups, which are those ones inside
                    # the current sphere.
    ...
    def _generate_pseudo_grps(self, atm_grps_mngr):
        ...
        for hydrop_grp in atm_grps_mngr.filter_by_types(["Hydrophobe"]):
            for inter in hydrop_grp.interactions:
                ...
                # Ignore non-hydrophobic interactions or interactions
                # already mapped.
                ...
                for atm_grp, atms in [src_tuple, trgt_tuple]:
                    ...
                    # a new one.
                    cf = ChemicalFeature("Hydrophobe")
                    pseudo_grp = \
                        pseudo_grps.get(atms,
                                        PseudoAtomGroup(atm_grp,
                                                        atms,
                                                        [cf]))
    ...
    def _get_derived_grps(self, atm_grp, pseudo_grps_mapping):
        ...
        derived_atm_grps = set([ag for a in atm_grp.atoms
                                for ag in a.atm_grps])

        # Get derived pseudo-groups for the informed atom group.
        derived_atm_grps.update(set([pseudo_grp for a in atm_grp.atoms
                                     for pseudo_grp
                                     in pseudo_grps_mapping.get((a,), [])]))
