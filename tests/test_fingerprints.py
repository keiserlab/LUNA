...
project_path = f"{LUNA_PATH}/tests/projects/test_fingerprints"
ifp_path = f"{project_path}/results/fingerprints/"
...
class FingerprintTest(unittest.TestCase):
    ...
    def _get_project_results(self):
        ...
        if not exists(proj_pkl_file):
            ...
            entries = [MolFileEntry.from_mol_file("protein",
                                                  lig_id,
                                                  mol_file=lig_file,
                                                  is_multimol_file=True,
                                                  autoload=True)
                       for lig_id in test_ligs]
    ...
    def _generate_manual_ifp(self,
                             agm,
                             ifp_num_levels=2,
                             ifp_radius_step=5.73171):

        shells = ShellGenerator(ifp_num_levels,
                                ifp_radius_step,
                                diff_comp_classes=True,
                                ifp_type=IFPType.EIFP)

        sm = shells.create_shells(agm)
        ifp = sm.to_fingerprint(count_fp=True,
                                unique_shells=False,
                                fold_to_length=4096)
        ifp_bits_str = "\t".join([str(idx) for idx in ifp.counts.keys()])
        ifp_count_str = "\t".join([str(count)
                                   for count in ifp.counts.values()])

        return "%s,%s,%s" % (agm.entry.to_string(),
                             ifp_bits_str,
                             ifp_count_str)
    ...
    def test_manual_ifps_creation(self):
        ...
        expected_results = {"atm_grps_len": [("protein:ZINC000012442563",
                                             ...
                                             ("protein:ZINC000343043015",
                                              422)],
                            "inter_len": [("protein:ZINC000012442563", 73),
                                          ("protein:ZINC000343043015", 52)],
        ...
        atm_grps_len = [(r.entry.to_string(), len(r.atm_grps_mngr))
                        for r in results]
        self.assertEqual(expected_results["atm_grps_len"], atm_grps_len)

        inter_len = [(r.entry.to_string(), len(r.interactions_mngr))
                     for r in results]
        ...
        idx = 0
        ifp_as_str = self._generate_manual_ifp(results[idx].atm_grps_mngr,
                                               ifp_num_levels=3)
